...
stb_uint
stb_compress(stb_uchar* out, stb_uchar* in, stb_uint len);

static bool
binary_to_compressed_c(const char* filename,
                       const char* symbol,
...
main(int argc, char** argv)
{
    ...
    bool ret = binary_to_compressed_c(argv[argn],
                                      argv[argn + 1],
                                      use_base85_encoding,
                                      use_compression,
                                      use_static);
                       ...
                       bool use_static)
{
    ...
    if (use_base85_encoding) {
        fprintf(out,
                "%sconst char %s_%sdata_base85[%d+1] =\n    \"",
                static_str,
                symbol,
                compressed_str,
        ...
        fprintf(out,
                "%sconst unsigned int %s_%ssize = %d;\n",
                static_str,
                symbol,
                compressed_str,
                (int)compressed_sz);
        fprintf(out,
                "%sconst unsigned int %s_%sdata[%d/4] =\n{",
                static_str,
                symbol,
                compressed_str,
...
static unsigned int
stb_matchlen(stb_uchar* m1, stb_uchar* m2, stb_uint maxlen)
{
    stb_uint i;
    for (i = 0; i < maxlen; ++i)
        if (m1[i] != m2[i])
...
static void
outliterals(stb_uchar* in, int numlit)
{
    while (numlit > 65536) {
...
static int
stb_not_crap(int best, int dist)
{
...
static int
stb_compress_chunk(stb_uchar* history,
                   stb_uchar* start,
                   stb_uchar* end,
                   int length,
...
static int
stb_compress_inner(stb_uchar* input, stb_uint length)
{
    int literals = 0;
    stb_uint len, i;
...
stb_uint
stb_compress(stb_uchar* out, stb_uchar* input, stb_uint length)
{
    stb__out = out;
