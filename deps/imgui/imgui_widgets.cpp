...
#pragma clang diagnostic ignored                                               \
  "-Wdeprecated-enum-enum-conversion" // warning: bitwise operation between
                                      ...
                                      // ('XXXFlags_' and 'XXXFlagsPrivate_') is
                                      // deprecated
#pragma clang diagnostic ignored                                               \
...
// For InputTextEx()
static bool
InputTextFilterCharacter(ImGuiContext* ctx,
                         unsigned int* p_char,
...
static int
InputTextCalcTextLenAndLineCount(const char* text_begin,
                                 const char** out_text_end);
static ImVec2
InputTextCalcTextSizeW(ImGuiContext* ctx,
...
void
ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
...
void
ImGui::TextUnformatted(const char* text, const char* text_end)
{
...
void
ImGui::TextV(const char* fmt, va_list args)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
...
void
ImGui::TextColored(const ImVec4& col, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
...
void
ImGui::TextColoredV(const ImVec4& col, const char* fmt, va_list args)
{
...
void
ImGui::TextDisabled(const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
...
void
ImGui::TextDisabledV(const char* fmt, va_list args)
{
    ImGuiContext& g = *GImGui;
...
void
ImGui::TextWrapped(const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
...
void
ImGui::TextWrappedV(const char* fmt, va_list args)
{
    ImGuiContext& g = *GImGui;
...
void
ImGui::LabelTextV(const char* label, const char* fmt, va_list args)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
...
void
ImGui::BulletText(const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
...
void
ImGui::BulletTextV(const char* fmt, va_list args)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
...
bool
ImGui::ButtonBehavior(const ImRect& bb,
                      ImGuiID id,
                      bool* out_hovered,
                      bool* out_held,
...
bool
ImGui::ButtonEx(const char* label,
                ...
                ImGuiButtonFlags flags)
{
    ...
    if ((flags & ImGuiButtonFlags_AlignTextBaseLine) &&
        style.FramePadding.y <
          window->DC
            .CurrLineTextBaseOffset) // Try to vertically align buttons that are
...
bool
ImGui::Button(const char* label, const ImVec2& size_arg)
{
...
bool
ImGui::InvisibleButton(const char* str_id,
                       const ImVec2& size_arg,
                       ImGuiButtonFlags flags)
{
    ImGuiContext& g = *GImGui;
...
bool
ImGui::ArrowButtonEx(const char* str_id,
                     ImGuiDir dir,
                     ImVec2 size,
                     ImGuiButtonFlags flags)
{
...
bool
ImGui::CloseButton(ImGuiID id, const ImVec2& pos)
{
    ImGuiContext& g = *GImGui;
...
bool
ImGui::CollapseButton(ImGuiID id, const ImVec2& pos)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
ImGuiID
ImGui::GetWindowScrollbarID(ImGuiWindow* window, ImGuiAxis axis)
{
...
ImRect
ImGui::GetWindowScrollbarRect(ImGuiWindow* window, ImGuiAxis axis)
{
    ...
    const float scrollbar_size =
      window
        ->ScrollbarSizes[axis ^ 1]; // (ScrollbarSizes.x = width of Y scrollbar;
...
bool
ImGui::ScrollbarEx(const ImRect& bb_frame,
                   ImGuiID id,
                   ImGuiAxis axis,
                   ImS64* p_scroll_v,
                   ImS64 size_avail_v,
...
void
ImGui::Image(ImTextureID user_texture_id,
             const ImVec2& size,
             const ImVec2& uv0,
             const ImVec2& uv1,
...
bool
ImGui::ImageButtonEx(ImGuiID id,
                     ...
                     ImGuiButtonFlags flags)
{
    ...
    window->DrawList->AddImage(texture_id,
                               bb.Min + padding,
                               bb.Max - padding,
                               uv0,
                               uv1,
...
bool
ImGui::ImageButton(const char* str_id,
...
bool
ImGui::ImageButton(ImTextureID user_texture_id,
                   const ImVec2& size,
                   const ImVec2& uv0,
                   const ImVec2& uv1,
                   int frame_padding,
...
template<typename T>
bool
ImGui::CheckboxFlagsT(const char* label, T* flags, T flags_value)
{
...
bool
ImGui::CheckboxFlags(const char* label, int* flags, int flags_value)
{
    return CheckboxFlagsT(label, flags, flags_value);
}

bool
ImGui::CheckboxFlags(const char* label,
                     unsigned int* flags,
...
bool
ImGui::CheckboxFlags(const char* label, ImS64* flags, ImS64 flags_value)
{
    return CheckboxFlagsT(label, flags, flags_value);
}

bool
ImGui::CheckboxFlags(const char* label, ImU64* flags, ImU64 flags_value)
{
    return CheckboxFlagsT(label, flags, flags_value);
}
...
ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    RenderFrame(
      bb.Min, bb.Max, GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);
    ...
    if (overlay_size.x > 0.0f)
        RenderTextClipped(
          ImVec2(ImClamp(fill_br.x + style.ItemSpacing.x,
                         bb.Min.x,
...
void
ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
...
void
ImGui::SeparatorTextEx(ImGuiID id,
                       const char* label,
...
bool
ImGui::SplitterBehavior(const ImRect& bb,
                        ImGuiID id,
                        ImGuiAxis axis,
                        float* size1,
                        float* size2,
...
ShrinkWidthItemComparer(const void* lhs, const void* rhs)
{
...
void
ImGui::ShrinkWidths(ImGuiShrinkWidthItem* items, int count, float width_excess)
{
    if (count == 1) {
...
static float
CalcMaxPopupHeightFromItemCount(int items_count)
{
    ImGuiContext& g = *GImGui;
    if (items_count <= 0)
        return FLT_MAX;
...
bool
ImGui::BeginCombo(const char* label,
...
bool
ImGui::BeginComboPopup(ImGuiID popup_id,
                       const ImRect& bb,
                       ImGuiComboFlags flags)
{
    ImGuiContext& g = *GImGui;
...
static const char*
Items_ArrayGetter(void* data, int idx)
{
...
static const char*
Items_SingleStringGetter(void* data, int idx)
{
...
static const char*
ImGuiGetNameFromIndexOldToNewCallback(void* user_data, int idx)
{
...
bool
ImGui::ListBox(const char* label,
               int* current_item,
...
const ImGuiDataTypeInfo*
ImGui::DataTypeGetInfo(ImGuiDataType data_type)
{
...
int
ImGui::DataTypeFormatString(char* buf,
                            int buf_size,
...
void
ImGui::DataTypeApplyOp(ImGuiDataType data_type,
                       int op,
                       void* output,
                       const void* arg1,
                       const void* arg2)
...
bool
ImGui::DataTypeApplyFromText(const char* buf,
                             ImGuiDataType data_type,
                             void* p_data,
                             const char* format)
{
...
int
ImGui::DataTypeCompare(ImGuiDataType data_type,
                       const void* arg_1,
                       const void* arg_2)
{
    switch (data_type) {
...
template<typename T>
static bool
DataTypeClampT(T* v, const T* v_min, const T* v_max)
{
...
bool
ImGui::DataTypeClamp(ImGuiDataType data_type,
                     void* p_data,
                     const void* p_min,
                     const void* p_max)
{
...
bool
ImGui::DragBehaviorT(ImGuiDataType data_type,
                     TYPE* v,
                     float v_speed,
                     const TYPE v_min,
                     const TYPE v_max,
...
bool
ImGui::DragScalar(const char* label,
                  ImGuiDataType data_type,
                  void* p_data,
                  float v_speed,
                  const void* p_min,
...
bool
ImGui::DragScalarN(const char* label,
                   ImGuiDataType data_type,
                   void* p_data,
                   int components,
                   float v_speed,
...
bool
ImGui::DragFloat(const char* label,
                 float* v,
                 float v_speed,
                 float v_min,
                 float v_max,
...
bool
ImGui::DragFloat2(const char* label,
                  float v[2],
                  float v_speed,
                  float v_min,
                  float v_max,
...
bool
ImGui::DragFloat3(const char* label,
                  float v[3],
                  float v_speed,
                  float v_min,
                  float v_max,
...
bool
ImGui::DragFloat4(const char* label,
                  float v[4],
                  float v_speed,
                  float v_min,
                  float v_max,
...
// this.
bool
ImGui::DragFloatRange2(const char* label,
                       float* v_current_min,
                       float* v_current_max,
                       float v_speed,
                       float v_min,
...
bool
ImGui::DragInt(const char* label,
               int* v,
               float v_speed,
               int v_min,
               int v_max,
...
bool
ImGui::DragInt2(const char* label,
                int v[2],
                float v_speed,
                int v_min,
                int v_max,
...
bool
ImGui::DragInt3(const char* label,
                int v[3],
                float v_speed,
                int v_min,
                int v_max,
...
bool
ImGui::DragInt4(const char* label,
                int v[4],
                float v_speed,
                int v_min,
                int v_max,
...
// this.
bool
ImGui::DragIntRange2(const char* label,
                     int* v_current_min,
                     int* v_current_max,
                     float v_speed,
                     int v_min,
...
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
float
ImGui::ScaleRatioFromValueT(ImGuiDataType data_type,
                            TYPE v,
                            TYPE v_min,
                            TYPE v_max,
                            bool is_logarithmic,
...
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
TYPE
ImGui::ScaleValueFromRatioT(ImGuiDataType data_type,
                            float t,
                            TYPE v_min,
                            TYPE v_max,
                            bool is_logarithmic,
...
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
bool
ImGui::SliderBehaviorT(const ImRect& bb,
                       ImGuiID id,
                       ImGuiDataType data_type,
                       TYPE* v,
                       const TYPE v_min,
        ...
        if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
            ...
            if (g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated) {
                                    ...
                                    // when already past the limits
                {
                    set_new_value = false;
                    g.SliderCurrentAccum =
                      0.0f; // If pushing up against the limits, don't continue
                            // to accumulate
                } else {
                    set_new_value = true;
...
bool
ImGui::SliderBehavior(const ImRect& bb,
                      ImGuiID id,
                      ImGuiDataType data_type,
                      void* p_v,
                      const void* p_min,
...
bool
ImGui::SliderScalar(const char* label,
                    ImGuiDataType data_type,
                    void* p_data,
                    const void* p_min,
                    const void* p_max,
...
bool
ImGui::SliderScalarN(const char* label,
                     ImGuiDataType data_type,
                     void* v,
                     int components,
                     const void* v_min,
...
bool
ImGui::SliderFloat(const char* label,
                   float* v,
                   float v_min,
                   float v_max,
                   const char* format,
...
bool
ImGui::SliderFloat2(const char* label,
                    float v[2],
                    float v_min,
                    float v_max,
                    const char* format,
...
bool
ImGui::SliderFloat3(const char* label,
                    float v[3],
                    float v_min,
                    float v_max,
                    const char* format,
...
bool
ImGui::SliderFloat4(const char* label,
                    float v[4],
                    float v_min,
                    float v_max,
                    const char* format,
...
bool
ImGui::SliderAngle(const char* label,
                   float* v_rad,
                   float v_degrees_min,
                   float v_degrees_max,
                   const char* format,
...
bool
ImGui::SliderInt(const char* label,
                 int* v,
                 int v_min,
                 int v_max,
                 const char* format,
...
bool
ImGui::SliderInt2(const char* label,
                  int v[2],
                  int v_min,
                  int v_max,
                  const char* format,
...
bool
ImGui::SliderInt3(const char* label,
                  int v[3],
                  int v_min,
                  int v_max,
                  const char* format,
...
bool
ImGui::SliderInt4(const char* label,
                  int v[4],
                  int v_min,
                  int v_max,
                  const char* format,
...
bool
ImGui::VSliderScalar(const char* label,
                     const ImVec2& size,
                     ImGuiDataType data_type,
                     void* p_data,
                     const void* p_min,
...
bool
ImGui::VSliderFloat(const char* label,
                    const ImVec2& size,
                    float* v,
                    float v_min,
                    float v_max,
...
bool
ImGui::VSliderInt(const char* label,
                  const ImVec2& size,
                  int* v,
                  int v_min,
                  int v_max,
...
// start with '%'
const char*
ImParseFormatFindStart(const char* fmt)
{
    while (char c = fmt[0]) {
        if (c == '%' && fmt[1] != '%')
            return fmt;
...
const char*
ImParseFormatFindEnd(const char* fmt)
{
...
void
ImParseFormatSanitizeForPrinting(const char* fmt_in,
                                 char* fmt_out,
                                 size_t fmt_out_size)
{
...
// "%07I64d" -> "%I64d"
const char*
ImParseFormatSanitizeForScanning(const char* fmt_in,
                                 char* fmt_out,
                                 size_t fmt_out_size)
{
    const char* fmt_end = ImParseFormatFindEnd(fmt_in);
...
template<typename TYPE>
static const char*
ImAtoi(const char* src, TYPE* output)
{
    int negative = 0;
    if (*src == '-') {
        negative = 1;
...
int
ImParseFormatPrecision(const char* fmt, int default_precision)
{
...
bool
ImGui::TempInputText(const ImRect& bb,
                     ImGuiID id,
                     const char* label,
                     char* buf,
                     int buf_size,
...
bool
ImGui::TempInputScalar(const ImRect& bb,
                       ImGuiID id,
                       const char* label,
                       ImGuiDataType data_type,
                       void* p_data,
...
bool
ImGui::InputScalar(const char* label,
                   ImGuiDataType data_type,
                   void* p_data,
                   const void* p_step,
                   ...
                   ImGuiInputTextFlags flags)
{
    ...
    if (p_step == NULL) {
        if (InputText(label, buf, IM_ARRAYSIZE(buf), flags))
            value_changed =
              DataTypeApplyFromText(buf, data_type, p_data, format);
    } else {
...
bool
ImGui::InputScalarN(const char* label,
                    ImGuiDataType data_type,
                    void* p_data,
                    int components,
                    const void* p_step,
...
bool
ImGui::InputFloat(const char* label,
                  float* v,
                  float step,
                  float step_fast,
                  const char* format,
...
bool
ImGui::InputFloat2(const char* label,
                   float v[2],
                   const char* format,
                   ImGuiInputTextFlags flags)
{
...
bool
ImGui::InputFloat3(const char* label,
                   float v[3],
                   const char* format,
                   ImGuiInputTextFlags flags)
{
...
bool
ImGui::InputFloat4(const char* label,
                   float v[4],
                   const char* format,
                   ImGuiInputTextFlags flags)
{
...
bool
ImGui::InputInt(const char* label,
                int* v,
                int step,
                int step_fast,
                ImGuiInputTextFlags flags)
...
bool
ImGui::InputInt2(const char* label, int v[2], ImGuiInputTextFlags flags)
{
    return InputScalarN(
...
bool
ImGui::InputInt3(const char* label, int v[3], ImGuiInputTextFlags flags)
{
    return InputScalarN(
...
bool
ImGui::InputInt4(const char* label, int v[4], ImGuiInputTextFlags flags)
{
    return InputScalarN(
...
bool
ImGui::InputDouble(const char* label,
                   double* v,
                   double step,
                   double step_fast,
                   const char* format,
...
bool
ImGui::InputText(const char* label,
                 char* buf,
                 size_t buf_size,
                 ImGuiInputTextFlags flags,
                 ImGuiInputTextCallback callback,
...
bool
ImGui::InputTextMultiline(const char* label,
                          char* buf,
                          size_t buf_size,
                          const ImVec2& size,
                          ImGuiInputTextFlags flags,
...
bool
ImGui::InputTextWithHint(const char* label,
                         const char* hint,
                         char* buf,
                         size_t buf_size,
                         ImGuiInputTextFlags flags,
...
static ImWchar
STB_TEXTEDIT_GETCHAR(const ImGuiInputTextState* obj, int idx)
{
    IM_ASSERT(idx <= obj->CurLenW);
    return obj->TextW[idx];
}
static float
STB_TEXTEDIT_GETWIDTH(ImGuiInputTextState* obj,
                      int line_start_idx,
                      int char_idx)
{
...
static void
STB_TEXTEDIT_LAYOUTROW(StbTexteditRow* r,
...
static int
is_word_boundary_from_right(ImGuiInputTextState* obj, int idx)
{
...
static int
is_word_boundary_from_left(ImGuiInputTextState* obj, int idx)
{
    if ((obj->Flags & ImGuiInputTextFlags_Password) || idx <= 0)
        return 0;
...
static int
STB_TEXTEDIT_MOVEWORDLEFT_IMPL(ImGuiInputTextState* obj, int idx)
{
    idx--;
...
static int
STB_TEXTEDIT_MOVEWORDRIGHT_MAC(ImGuiInputTextState* obj, int idx)
{
    idx++;
    int len = obj->CurLenW;
...
static int
STB_TEXTEDIT_MOVEWORDRIGHT_WIN(ImGuiInputTextState* obj, int idx)
{
    idx++;
    int len = obj->CurLenW;
...
static int
STB_TEXTEDIT_MOVEWORDRIGHT_IMPL(ImGuiInputTextState* obj, int idx)
{
    ImGuiContext& g = *obj->Ctx;
...
static void
STB_TEXTEDIT_DELETECHARS(ImGuiInputTextState* obj, int pos, int n)
{
...
static bool
STB_TEXTEDIT_INSERTCHARS(ImGuiInputTextState* obj,
                         int pos,
...
static void
stb_textedit_replace(ImGuiInputTextState* str,
...
void
ImGuiInputTextState::OnKeyPressed(int key)
{
...
// nuisance.
void
ImGuiInputTextCallbackData::DeleteChars(int pos, int bytes_count)
{
...
void
ImGuiInputTextCallbackData::InsertChars(int pos,
                                        const char* new_text,
                                        const char* new_text_end)
{
    ...
    if (new_text_len + BufTextLen >= BufSize) {
        ...
        int new_buf_size =
          BufTextLen + ImClamp(new_text_len * 4, 32, ImMax(256, new_text_len)) +
          1;
...
static void
InputTextReconcileUndoStateAfterUserCallback(ImGuiInputTextState* state,
                                             ...
                                             int new_length_a)
{
    ...
    ImTextStrFromUtf8(
      new_buf, new_length + 1, new_buf_a, new_buf_a + new_length_a);
...
void
ImGui::InputTextDeactivateHook(ImGuiID id)
{
    ImGuiContext& g = *GImGui;
...
bool
ImGui::InputTextEx(const char* label,
                   const char* hint,
                   char* buf,
                   int buf_size,
                   const ImVec2& size_arg,
                   ...
                   void* callback_user_data)
{
    ...
    if (is_multiline) {
        ...
        bool child_visible = BeginChildEx(
          label, id, frame_bb.GetSize(), true, ImGuiWindowFlags_NoMove);
    ...
    const bool init_changed_specs =
      (state != NULL && state->Stb.single_line !=
            ...
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion |
                ...
                if (event_flag) {
                    ...
                    callback_buf =
                      is_readonly
                        ? buf
                        : state->TextA.Data; // Pointer may have been
                                             // invalidated by a resize callback
    ...
    if (render_cursor || render_selection) {
        ...
        if (render_selection) {
            ...
            ImU32 bg_color = GetColorU32(
              ...
              render_cursor
                ? 1.0f
                : 0.6f); // FIXME: current code flow mandate that render_cursor
                         // is always true here, we are leaving the transparent
                         // one for tests.
            float bg_offy_up =
              is_multiline
                ? 0.0f
            ...
            for (const ImWchar* p = text_selected_begin;
                ...
                if (rect_pos.y < clip_rect.y) {
                    ...
                    if (rect_size.x <= 0.0f)
                        rect_size.x =
                          IM_TRUNC(g.Font->GetCharAdvance((ImWchar)' ') *
        ...
        if (render_cursor) {
            ...
            if (!is_readonly) {
                ...
                g.PlatformImeData.InputPos = ImVec2(
                  cursor_screen_pos.x - 1.0f, cursor_screen_pos.y - g.FontSize);
...
void
ImGui::DebugNodeInputTextState(ImGuiInputTextState* state)
{
    ...
    if (BeginChild("undopoints",
                   ...
                   ImGuiChildFlags_Border)) // Visualize undo state
    {
...
static void
ColorEditRestoreH(const float* col, float* H)
{
    ImGuiContext& g = *GImGui;
...
// resetting.
static void
ColorEditRestoreHS(const float* col, float* H, float* S, float* V)
{
    ImGuiContext& g = *GImGui;
    IM_ASSERT(g.ColorEditCurrentID != 0);
    if (g.ColorEditSavedID != g.ColorEditCurrentID ||
...
ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB |
        ...
        const float w_item_one = ImMax(
          1.0f,
          IM_TRUNC((w_inputs - (style.ItemInnerSpacing.x) * (components - 1)) /
                   (float)components));
        const float w_item_last =
          ImMax(1.0f,
                IM_TRUNC(w_inputs - (w_item_one + style.ItemInnerSpacing.x) *
                                      (components - 1)));
...
bool
ImGui::ColorPicker3(const char* label, float col[3], ImGuiColorEditFlags flags)
{
...
static void
RenderArrowsForVerticalBar(ImDrawList* draw_list,
                           ImVec2 pos,
                           ImVec2 half_sz,
                           float bar_w,
                           float alpha)
...
bool
ImGui::ColorPicker4(const char* label,
                    float col[4],
...
bool
ImGui::ColorButton(const char* desc_id,
                   const ImVec4& col,
                   ImGuiColorEditFlags flags,
                   const ImVec2& size_arg)
{
...
void
ImGui::ColorTooltip(const char* text,
                    const float* col,
                    ImGuiColorEditFlags flags)
{
    ImGuiContext& g = *GImGui;
...
void
ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags)
{
...
void
ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
    ...
    if (allow_opt_alpha_bar) {
        if (allow_opt_picker)
            Separator();
        CheckboxFlags(
          "Alpha Bar", &g.ColorEditOptions, ImGuiColorEditFlags_AlphaBar);
    }
    EndPopup();
    g.LockMarkEdited--;
}
...
bool
ImGui::TreeNode(const void* ptr_id, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
...
bool
ImGui::TreeNodeV(const char* str_id, const char* fmt, va_list args)
{
    return TreeNodeExV(str_id, 0, fmt, args);
}

bool
ImGui::TreeNodeV(const void* ptr_id, const char* fmt, va_list args)
{
    return TreeNodeExV(ptr_id, 0, fmt, args);
}

bool
ImGui::TreeNodeEx(const char* label, ImGuiTreeNodeFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool
ImGui::TreeNodeEx(const char* str_id,
...
bool
ImGui::TreeNodeEx(const void* ptr_id,
                  ImGuiTreeNodeFlags flags,
                  const char* fmt,
                  ...)
{
...
bool
ImGui::TreeNodeExV(const char* str_id,
                   ImGuiTreeNodeFlags flags,
                   const char* fmt,
                   va_list args)
{
...
bool
ImGui::TreeNodeExV(const void* ptr_id,
                   ImGuiTreeNodeFlags flags,
                   const char* fmt,
                   va_list args)
{
...
void
ImGui::TreeNodeSetOpen(ImGuiID id, bool open)
{
    ImGuiContext& g = *GImGui;
...
bool
ImGui::TreeNodeUpdateNextOpen(ImGuiID id, ImGuiTreeNodeFlags flags)
{
...
bool
ImGui::TreeNodeBehavior(ImGuiID id,
                        ImGuiTreeNodeFlags flags,
                        const char* label,
                        const char* label_end)
{
...
void
ImGui::TreePushOverrideID(ImGuiID id)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    Indent();
...
bool
ImGui::CollapsingHeader(const char* label, ImGuiTreeNodeFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
static int
ImStrimatchlen(const char* s1, const char* s1_end, const char* s2)
{
    int match_len = 0;
...
int
ImGui::TypingSelectFindMatch(ImGuiTypingSelectRequest* req,
                             int items_count,
...
int
ImGui::TypingSelectFindNextSingleCharMatch(
...
int
ImGui::TypingSelectFindBestLeadingMatch(ImGuiTypingSelectRequest* req,
                                        int items_count,
...
void
ImGui::DebugNodeTypingSelectState(ImGuiTypingSelectState* data)
{
#ifndef IMGUI_DISABLE_DEBUG_TOOLS
...
// item_height).
bool
ImGui::BeginListBox(const char* label, const ImVec2& size_arg)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
...
int
ImGui::PlotEx(ImGuiPlotType plot_type,
              const char* label,
...
static float
Plot_ArrayGetter(void* data, int idx)
{
...
void
ImGui::PlotLines(const char* label,
                 const float* values,
                 int values_count,
                 int values_offset,
                 const char* overlay_text,
...
void
ImGui::PlotHistogram(const char* label,
                     const float* values,
                     int values_count,
                     int values_offset,
                     const char* overlay_text,
...
void
ImGui::Value(const char* prefix, float v, const char* float_format)
{
    if (float_format) {
        char fmt[64];
...
void
ImGuiMenuColumns::Update(float spacing, bool window_reappearing)
{
...
void
ImGuiMenuColumns::CalcNextTotalWidth(bool update_offsets)
{
    ImU16 offset = 0;
...
float
ImGuiMenuColumns::DeclColumns(float w_icon,
                              float w_label,
                              float w_shortcut,
                              float w_mark)
{
    ...
    if (NavMoveRequestButNoResultYet() &&
        ...
        if (nav_earliest_child->ParentWindow == window &&
            ...
            g.NavDisableHighlight =
              true; // Hide highlight for the current frame so we don't see the
...
bool
ImGui::BeginViewportSideBar(const char* name,
...
ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        RenderArrow(
          window->DrawList,
          pos +
            ImVec2(offsets->OffsetMark + extra_w + g.FontSize * 0.30f, 0.0f),
          GetColorU32(ImGuiCol_Text),
          ImGuiDir_Right);
    }
    if (!enabled)
                                ...
                                // (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (g.HoveredWindow == window && child_menu_window != NULL) {
            ...
            tb.y =
              ta.y + ImMax((tb.y - pad_farmost_h) - ta.y,
                           -ref_unit *
    ...
    if (want_open && !menu_is_open &&
        ...
        // Don't reopen/recycle same menu level in the same frame, first close
...
bool
ImGui::MenuItemEx(const char* label,
                  const char* icon,
                  const char* shortcut,
                  bool selected,
                  bool enabled)
...
bool
ImGui::MenuItem(const char* label,
                const char* shortcut,
                bool selected,
                bool enabled)
{
...
namespace ImGui {
static void
TabBarLayout(ImGuiTabBar* tab_bar);
static ImU32
TabBarCalcTabID(ImGuiTabBar* tab_bar,
                const char* label,
                ImGuiWindow* docked_window);
static float
TabBarCalcMaxTabWidth();
static float
TabBarScrollClamp(ImGuiTabBar* tab_bar, float scrolling);
static void
TabBarScrollToTab(ImGuiTabBar* tab_bar,
                  ImGuiID tab_id,
                  ImGuiTabBarSection* sections);
static ImGuiTabItem*
TabBarScrollingButtons(ImGuiTabBar* tab_bar);
static ImGuiTabItem*
TabBarTabListPopupButton(ImGuiTabBar* tab_bar);
}
...
static inline int
TabItemGetSectionIdx(const ImGuiTabItem* tab)
{
...
static int IMGUI_CDECL
TabItemComparerBySection(const void* lhs, const void* rhs)
{
...
static ImGuiTabBar*
GetTabBarFromTabBarRef(const ImGuiPtrOrIndex& ref)
{
    ImGuiContext& g = *GImGui;
    return ref.Ptr ? (ImGuiTabBar*)ref.Ptr : g.TabBars.GetByIndex(ref.Index);
}

static ImGuiPtrOrIndex
GetTabBarRefFromTabBar(ImGuiTabBar* tab_bar)
{
    ImGuiContext& g = *GImGui;
...
bool
ImGui::BeginTabBar(const char* str_id, ImGuiTabBarFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
...
bool
ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar,
...
// not scrolling)
static float
TabBarCalcScrollableWidth(ImGuiTabBar* tab_bar, ImGuiTabBarSection* sections)
{
...
static void
ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ImGuiContext& g = *GImGui;
...
// the ID stack.
static ImU32
ImGui::TabBarCalcTabID(ImGuiTabBar* tab_bar,
                       const char* label,
                       ImGuiWindow* docked_window)
{
...
ImGuiTabItem*
ImGui::TabBarFindTabByID(ImGuiTabBar* tab_bar, ImGuiID tab_id)
{
    if (tab_id != 0)
...
ImGuiTabItem*
ImGui::TabBarFindTabByOrder(ImGuiTabBar* tab_bar, int order)
{
...
ImGuiTabItem*
ImGui::TabBarGetCurrentTab(ImGuiTabBar* tab_bar)
{
...
const char*
ImGui::TabBarGetTabName(ImGuiTabBar* tab_bar, ImGuiTabItem* tab)
{
    if (tab->NameOffset == -1)
        return "N/A";
...
void
ImGui::TabBarRemoveTab(ImGuiTabBar* tab_bar, ImGuiID tab_id)
{
...
void
ImGui::TabBarCloseTab(ImGuiTabBar* tab_bar, ImGuiTabItem* tab)
{
...
static float
ImGui::TabBarScrollClamp(ImGuiTabBar* tab_bar, float scrolling)
{
    scrolling =
...
// keys
static void
ImGui::TabBarScrollToTab(ImGuiTabBar* tab_bar,
                         ImGuiID tab_id,
                         ImGuiTabBarSection* sections)
{
...
void
ImGui::TabBarQueueFocus(ImGuiTabBar* tab_bar, ImGuiTabItem* tab)
{
    tab_bar->NextSelectedTabId = tab->ID;
}

void
ImGui::TabBarQueueReorder(ImGuiTabBar* tab_bar, ImGuiTabItem* tab, int offset)
{
    IM_ASSERT(offset != 0);
...
void
ImGui::TabBarQueueReorderFromMousePos(ImGuiTabBar* tab_bar,
...
bool
ImGui::TabBarProcessReorder(ImGuiTabBar* tab_bar)
{
    ImGuiTabItem* tab1 =
...
static ImGuiTabItem*
ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
static ImGuiTabItem*
ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    const ImVec2 backup_cursor_pos = window->DC.CursorPos;
    window->DC.CursorPos = ImVec2(
      tab_bar->BarRect.Min.x - g.Style.FramePadding.y, tab_bar->BarRect.Min.y);
...
bool
ImGui::BeginTabItem(const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
...
bool
ImGui::TabItemButton(const char* label, ImGuiTabItemFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
...
bool
ImGui::TabItemEx(ImGuiTabBar* tab_bar,
                 const char* label,
                 bool* p_open,
...
void
ImGui::SetTabItemClosed(const char* label)
{
    ImGuiContext& g = *GImGui;
                       ...
                       bool has_close_button_or_unsaved_marker)
{
    ...
    if (has_close_button_or_unsaved_marker)
        size.x +=
          g.Style.FramePadding.x + (g.Style.ItemInnerSpacing.x +
                                    g.FontSize); // We use Y intentionally to
                                                 // fit the close button circle.
    else
...
ImVec2
ImGui::TabItemCalcSize(ImGuiWindow*)
{
    IM_ASSERT(
...
void
ImGui::TabItemBackground(ImDrawList* draw_list,
                         const ImRect& bb,
                         ImGuiTabItemFlags flags,
                         ImU32 col)
{
    ...
    if (g.Style.TabBorderSize > 0.0f) {
        ...
        draw_list->PathStroke(
          GetColorU32(ImGuiCol_Border), 0, g.Style.TabBorderSize);
    }
}
...
void
ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list,
                                  const ImRect& bb,
                                  ImGuiTabItemFlags flags,
                                  ...
                                  bool* out_text_clipped)
{
    ...
    if (close_button_visible) {
        ...
        RenderBullet(
          draw_list, bullet_bb.GetCenter(), GetColorU32(ImGuiCol_Text));
    }
