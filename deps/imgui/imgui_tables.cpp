...
static const float TABLE_RESIZE_SEPARATOR_FEEDBACK_TIMER =
  0.06f; // Delay/timer before making the hover feedback (color+cursor) visible
         // because tables/columns tends to be more cramped.

// Helper
inline ImGuiTableFlags
...
ImGuiTable*
ImGui::TableFindByID(ImGuiID id)
{
...
bool
ImGui::BeginTable(const char* str_id,
                  int columns_count,
...
bool
ImGui::BeginTableEx(const char* name,
                    ImGuiID id,
                    int columns_count,
                    ImGuiTableFlags flags,
                    const ImVec2& outer_size,
                    float inner_width)
{
    ...
    if (instance_no > 0) {
        ...
        instance_id = GetIDWithSeed(
          instance_no,
          GetIDWithSeed(
            "##Instances", NULL, id)); // Push "##Instances" followed by
    ...
    if (use_child_window) {
        ...
        BeginChildEx(
          name, instance_id, outer_rect.GetSize(), false, child_flags);
            ...
            if (old_columns_to_preserve && n < old_columns_count) {
                ...
                *column = ImGuiTableColumn();
                ...
                column->IsPreserveWidthAuto =
                  true; // Preserve WidthAuto when reinitializing a live table:
                        // not technically necessary but remove a visible
                        // flicker
...
void
ImGui::TableBeginInitMemory(ImGuiTable* table, int columns_count)
{
...
// Apply queued resizing/reordering/hiding requests
void
ImGui::TableBeginApplyRequests(ImGuiTable* table)
{
    ...
    if (table->InstanceCurrent == 0) {
        ...
        if (table->ReorderColumn != -1 && table->ReorderColumnDir != 0) {
            ...
            ImGuiTableColumn* dst_column =
              &table
                 ->Columns[(reorder_dir == -1) ? src_column->PrevEnabledColumn
            ...
            for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
                table
                  ->DisplayOrderToIndex[table->Columns[column_n].DisplayOrder] =
...
// extending
static void
TableSetupColumnFlags(ImGuiTable* table,
...
// _WidthAuto columns?
void
ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ImGuiContext& g = *GImGui;
    ...
    if (table->FreezeColumnsRequest > 0)
        table->InnerWindow->DecoInnerSizeX1 =
          table
            ->Columns[table
                        ->DisplayOrderToIndex[table->FreezeColumnsRequest - 1]]
            .MaxX -
          table->OuterRect.Min.x;
...
// feedback noise.
void
ImGui::TableUpdateBorders(ImGuiTable* table)
{
    ImGuiContext& g = *GImGui;
...
// [Public]
void
ImGui::TableSetupScrollFreeze(int columns, int rows)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    IM_ASSERT(table != NULL &&
...
const char*
ImGui::TableGetColumnName(int column_n)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    if (!table)
...
const char*
ImGui::TableGetColumnName(const ImGuiTable* table, int column_n)
{
...
void
ImGui::TableSetColumnEnabled(int column_n, bool enabled)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    IM_ASSERT(table != NULL);
...
ImGui::TableGetColumnFlags(int column_n)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    if (!table)
...
ImRect
ImGui::TableGetCellBgRect(const ImGuiTable* table, int column_n)
{
    const ImGuiTableColumn* column = &table->Columns[column_n];
...
ImGuiID
ImGui::TableGetColumnResizeID(ImGuiTable* table, int column_n, int instance_no)
{
    IM_ASSERT(column_n >= 0 && column_n < table->ColumnsCount);
...
void
ImGui::TableNextRow(ImGuiTableRowFlags row_flags, float row_min_height)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
...
void
ImGui::TableBeginRow(ImGuiTable* table)
{
...
void
ImGui::TableEndRow(ImGuiTable* table)
{
    ImGuiContext& g = *GImGui;
    ...
    if (unfreeze_rows_actual) {
        ...
        // Update cliprect ahead of TableBeginCell() so clipper can access to
        // new ClipRect->Min.y
...
bool
ImGui::TableSetColumnIndex(int column_n)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    if (!table)
...
// clipped columns.
void
ImGui::TableBeginCell(ImGuiTable* table, int column_n)
{
    ImGuiContext& g = *GImGui;
    ImGuiTableColumn* column = &table->Columns[column_n];
    ImGuiWindow* window = table->InnerWindow;
    ...
    window->DC.CursorPosPrevLine.x =
      window->DC.CursorPos
        .x; // PrevLine.y is preserved. This allows users to call SameLine() to
...
void
ImGui::TableEndCell(ImGuiTable* table)
{
    ...
    if (table->RowFlags & ImGuiTableRowFlags_Headers)
        p_max_pos_x =
          &column->ContentMaxXHeadersUsed; // Useful in case user submit
                                           // contents in header row that is not
                                           // a TableHeader() call
    else
    ...
    *p_max_pos_x = ImMax(*p_max_pos_x, window->DC.CursorMaxPos.x);
    if (column->IsEnabled)
        table->RowPosY2 = ImMax(
          table->RowPosY2, window->DC.CursorMaxPos.y + table->RowCellPaddingY);
...
float
ImGui::TableGetMaxColumnWidth(const ImGuiTable* table, int column_n)
{
    const ImGuiTableColumn* column = &table->Columns[column_n];
...
// the WidthAuto field
float
ImGui::TableGetColumnWidthAuto(ImGuiTable* table, ImGuiTableColumn* column)
{
...
void
ImGui::TableSetColumnWidth(int column_n, float width)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
...
void
ImGui::TableSetColumnWidthAutoSingle(ImGuiTable* table, int column_n)
{
...
void
ImGui::TableSetColumnWidthAutoAll(ImGuiTable* table)
{
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
        ImGuiTableColumn* column = &table->Columns[column_n];
        if (!column->IsEnabled &&
...
void
ImGui::TableUpdateColumnsWeightFromWidth(ImGuiTable* table)
{
...
void
ImGui::TableSetupDrawChannels(ImGuiTable* table)
{
...
void
ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ImGuiContext& g = *GImGui;
    ImDrawListSplitter* splitter = table->DrawSplitter;
...
void
ImGui::TableDrawBorders(ImGuiTable* table)
{
    ImGuiWindow* inner_window = table->InnerWindow;
...
void
ImGui::TableFixColumnSortDirection(ImGuiTable* table, ImGuiTableColumn* column)
{
    if (column->SortOrder == -1 ||
...
void
ImGui::TableSetColumnSortDirection(int column_n,
...
void
ImGui::TableSortSpecsSanitize(ImGuiTable* table)
{
...
void
ImGui::TableSortSpecsBuild(ImGuiTable* table)
{
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
        if (IM_BITARRAY_TESTBIT(table->EnabledMaskByIndex, column_n))
            if (table->Columns[column_n].Flags &
                ...
                width = ImMax(
                  width,
                  CalcTextSize(TableGetColumnName(table, column_n), NULL, true)
                    .x);
...
void
ImGui::TableAngledHeadersRowEx(float angle, float max_label_width)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    if (table->FreezeColumnsCount > 0)
        clip_rect_min_x = ImMax(
          clip_rect_min_x, table->Columns[table->FreezeColumnsCount - 1].MaxX);
    TableSetBgColor(ImGuiTableBgTarget_RowBg0, 0); // Cancel
    PushClipRect(
      table->BgClipRect.Min, table->BgClipRect.Max, false); // Span all columns
    ...
    for (int pass = 0; pass < 2; pass++)
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
            ...
            if (pass == 0) {
                ...
                // bg_shape[3], GetColorU32(ImGuiCol_TableBorderLight), 1.0f);
                max_x = ImMax(max_x, bg_shape[3].x);

                // Draw label (first draw at an offset where RenderTextXXX()
                // function won't meddle with applying current ClipRect, then
                // transform to final offset)
                // FIXME: May be worth tidying up all those operations to make
                ...
                RenderTextEllipsis(draw_list,
                                   ...
                                   NULL,
                                   &label_size);
                // if (g.IO.KeyShift) { draw_list->AddRect(label_r.Min,
                // label_r.Max, IM_COL32(0, 255, 0, 255), 0.0f, 0, 2.0f); }
                int vtx_idx_end = draw_list->_VtxCurrentIdx;

                // Rotate and offset label
                ImVec2 pivot_in = label_r.GetBL();
                ImVec2 pivot_out = ImVec2(column->WorkMinX, row_r.Max.y) +
                                   (flip_label ? (unit_right * clip_width)
...
void
ImGui::TableOpenContextMenu(int column_n)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    if (column_n == -1 &&
...
bool
ImGui::TableBeginContextMenuPopup(ImGuiTable* table)
{
...
void
ImGui::TableDrawDefaultContextMenu(ImGuiTable* table,
                                   ImGuiTableFlags flags_for_section_to_display)
{
    ...
    if (flags_for_section_to_display & ImGuiTableFlags_Hideable) {
        ...
        for (int other_column_n = 0; other_column_n < table->ColumnsCount;
            ...
            if (MenuItem(
                  name, NULL, other_column->IsUserEnabled, menu_item_active))
...
static void
TableSettingsInit(ImGuiTableSettings* settings,
                  ImGuiID id,
                  int columns_count,
...
static size_t
TableSettingsCalcChunkSize(int columns_count)
{
...
ImGuiTableSettings*
ImGui::TableSettingsCreate(ImGuiID id, int columns_count)
{
    ImGuiContext& g = *GImGui;
...
ImGuiTableSettings*
ImGui::TableSettingsFindByID(ImGuiID id)
{
...
ImGuiTableSettings*
ImGui::TableGetBoundSettings(ImGuiTable* table)
{
...
void
ImGui::TableResetSettings(ImGuiTable* table)
{
...
void
ImGui::TableLoadSettings(ImGuiTable* table)
{
    ImGuiContext& g = *GImGui;
...
static void
TableSettingsHandler_ClearAll(ImGuiContext* ctx, ImGuiSettingsHandler*)
{
    ImGuiContext& g = *ctx;
...
static void
TableSettingsHandler_ApplyAll(ImGuiContext* ctx, ImGuiSettingsHandler*)
{
    ImGuiContext& g = *ctx;
    for (int i = 0; i != g.Tables.GetMapSize(); i++)
        if (ImGuiTable* table = g.Tables.TryGetMapData(i)) {
...
static void*
TableSettingsHandler_ReadOpen(ImGuiContext*,
...
static void
TableSettingsHandler_ReadLine(ImGuiContext*,
                              ImGuiSettingsHandler*,
                              void* entry,
                              const char* line)
{
...
static void
TableSettingsHandler_WriteAll(ImGuiContext* ctx,
...
void
ImGui::TableRemove(ImGuiTable* table)
{
...
void
ImGui::TableGcCompactTransientBuffers(ImGuiTable* table)
{
...
void
ImGui::TableGcCompactTransientBuffers(ImGuiTableTempData* temp_data)
{
...
void
ImGui::DebugNodeTable(ImGuiTable* table)
{
...
void
ImGui::DebugNodeTableSettings(ImGuiTableSettings* settings)
{
...
void
ImGui::DebugNodeTable(ImGuiTable*)
{
}
void
ImGui::DebugNodeTableSettings(ImGuiTableSettings*)
{
}
...
void
ImGui::SetWindowClipRectBeforeSetChannel(ImGuiWindow* window,
...
float
ImGui::GetColumnOffsetFromNorm(const ImGuiOldColumns* columns,
                               float offset_norm)
{
...
float
ImGui::GetColumnNormFromOffset(const ImGuiOldColumns* columns, float offset)
{
...
static float
GetDraggedColumnOffset(ImGuiOldColumns* columns, int column_index)
{
...
float
ImGui::GetColumnOffset(int column_index)
{
    ImGuiWindow* window = GetCurrentWindowRead();
...
static float
GetColumnWidthEx(ImGuiOldColumns* columns,
                 int column_index,
...
float
ImGui::GetColumnWidth(int column_index)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ImGuiOldColumns* columns = window->DC.CurrentColumns;
...
void
ImGui::SetColumnOffset(int column_index, float offset)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ImGuiOldColumns* columns = window->DC.CurrentColumns;
...
void
ImGui::PushColumnClipRect(int column_index)
{
    ImGuiWindow* window = GetCurrentWindowRead();
    ImGuiOldColumns* columns = window->DC.CurrentColumns;
    if (column_index < 0)
...
ImGuiOldColumns*
ImGui::FindOrCreateColumns(ImGuiWindow* window, ImGuiID id)
{
...
ImGuiID
ImGui::GetColumnsID(const char* str_id, int columns_count)
{
...
void
ImGui::BeginColumns(const char* str_id,
                    int columns_count,
...
void
ImGui::Columns(int columns_count, const char* id, bool border)
{
    ImGuiWindow* window = GetCurrentWindow();
    IM_ASSERT(columns_count >= 1);
