...
extern ImGuiID
ImHashData(const void* data_p, size_t data_size, ImU32 seed = 0);
...
static void
SafeRelease(WGPUBindGroupLayout& res)
{
    if (res)
...
static void
SafeRelease(WGPURenderPipeline& res)
{
    if (res)
...
static void
SafeRelease(WGPUShaderModule& res)
{
    if (res)
...
static void
SafeRelease(WGPUTextureView& res)
{
    if (res)
...
static void
SafeRelease(RenderResources& res)
{
...
static void
SafeRelease(FrameResources& res)
{
...
static void
ImGui_ImplWGPU_SetupRenderState(ImDrawData* draw_data,
                                ...
                                FrameResources* fr)
{
    ...
    // draw_data->DisplayPos+data_data->DisplaySize (bottom right).
    {
        ...
        wgpuQueueWriteBuffer(bd->defaultQueue,
                             bd->renderResources.Uniforms,
                             offsetof(Uniforms, MVP),
                             mvp,
                             sizeof(Uniforms::MVP));
        float gamma;
                              ...
                              WGPURenderPassEncoder pass_encoder)
{
    ...
    wgpuQueueWriteBuffer(
      bd->defaultQueue, fr->IndexBuffer, 0, fr->IndexBufferHost, ib_write_size);
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
            ...
            if (pcmd->UserCallback != nullptr) {
                ...
                if (pcmd->UserCallback == ImDrawCallback_ResetRenderState)
                    ImGui_ImplWGPU_SetupRenderState(
...
ImGui_ImplWGPU_CreateFontsTexture()
{
    ...
    // Upload texture to graphics system
    {
        ...
        bd->renderResources.FontTextureView = wgpuTextureCreateView(
          bd->renderResources.FontTexture, &tex_view_desc);
    }
    ...
    // Upload texture data
    {
        ...
        wgpuQueueWriteTexture(bd->defaultQueue,
                              &dst_view,
                              pixels,
                              (uint32_t)(width * size_pp * height),
                              &layout,
                              &size);
    }
...
bool
ImGui_ImplWGPU_Init(WGPUDevice device,
