...
static void
ImGui_ImplWin32_AddKeyEvent(ImGuiKey key,
                            bool down,
                            int native_keycode,
...
ImGui_ImplWin32_UpdateGamepads()
{
    ...
    if (bd->WantUpdateHasGamepad) {
        ...
        bd->HasGamepad = bd->XInputGetCapabilities
                           ? (bd->XInputGetCapabilities(
                                0, XINPUT_FLAG_GAMEPAD, &caps) == ERROR_SUCCESS)
                           : false;
...
static ImGuiKey
ImGui_ImplWin32_VirtualKeyToImGuiKey(WPARAM wParam)
{
    switch (wParam) {
        case VK_TAB:
            return ImGuiKey_Tab;
    ...
    switch (msg) {
        ...
        case WM_NCMOUSEMOVE: {
            ...
            if (bd->MouseTrackedArea != area) {
                TRACKMOUSEEVENT tme_cancel = {
                    sizeof(tme_cancel), TME_CANCEL, hwnd, 0
                };
        ...
        case WM_SYSKEYUP: {
            ...
            if (wParam < 256) {
                ...
                // (keypad enter doesn't have its own... VK_RETURN with
                // KF_EXTENDED flag means keypad enter, see IM_VK_KEYPAD_ENTER
                // definition for details, it is mapped to
                ...
                if (vk == VK_SHIFT) {
                    ...
                    if (IsVkDown(VK_RSHIFT) == is_key_down) {
                        ImGui_ImplWin32_AddKeyEvent(ImGuiKey_RightShift,
                                                    is_key_down,
                                                    VK_RSHIFT,
                                                    scancode);
                    }
                    ...
                    if (IsVkDown(VK_LCONTROL) == is_key_down) {
                        ImGui_ImplWin32_AddKeyEvent(ImGuiKey_LeftCtrl,
                                                    is_key_down,
                                                    VK_LCONTROL,
                                                    scancode);
                    }
                    if (IsVkDown(VK_RCONTROL) == is_key_down) {
                        ImGui_ImplWin32_AddKeyEvent(ImGuiKey_RightCtrl,
                                                    is_key_down,
                                                    VK_RCONTROL,
                                                    scancode);
                    }
...
#define _IsWindowsVistaOrGreater()                                             \
    _IsWindowsVersionOrGreater(                                                \
      HIBYTE(0x0600), LOBYTE(0x0600), 0) // _WIN32_WINNT_VISTA
#define _IsWindows8OrGreater()                                                 \
    _IsWindowsVersionOrGreater(                                                \
      HIBYTE(0x0602), LOBYTE(0x0602), 0) // _WIN32_WINNT_WIN8
#define _IsWindows8Point1OrGreater()                                           \
    _IsWindowsVersionOrGreater(                                                \
      HIBYTE(0x0603), LOBYTE(0x0603), 0) // _WIN32_WINNT_WINBLUE
