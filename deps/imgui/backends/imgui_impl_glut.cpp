...
static ImGuiKey
ImGui_ImplGLUT_KeyToImGuiKey(int key)
{
    switch (key) {
        case '\t':
            return ImGuiKey_Tab;
...
static void
ImGui_ImplGLUT_AddKeyEvent(ImGuiKey key, bool down, int native_keycode)
{
    ImGuiIO& io = ImGui::GetIO();
    io.AddKeyEvent(key, down);
    io.SetKeyEventNativeData(
      key, native_keycode, -1); // To support legacy indexing (<1.87 user code)
}

void
ImGui_ImplGLUT_KeyboardFunc(unsigned char c, int x, int y)
{
...
void
ImGui_ImplGLUT_KeyboardUpFunc(unsigned char c, int x, int y)
{
...
void
ImGui_ImplGLUT_SpecialFunc(int key, int x, int y)
{
...
void
ImGui_ImplGLUT_SpecialUpFunc(int key, int x, int y)
{
...
void
ImGui_ImplGLUT_MouseFunc(int glut_button, int state, int x, int y)
{
    ImGuiIO& io = ImGui::GetIO();
...
#ifdef __FREEGLUT_EXT_H__
void
ImGui_ImplGLUT_MouseWheelFunc(int button, int dir, int x, int y)
{
    ImGuiIO& io = ImGui::GetIO();
    io.AddMousePosEvent((float)x, (float)y);
    if (dir != 0)
...
void
ImGui_ImplGLUT_ReshapeFunc(int w, int h)
{
    ImGuiIO& io = ImGui::GetIO();
...
void
ImGui_ImplGLUT_MotionFunc(int x, int y)
{
    ImGuiIO& io = ImGui::GetIO();
    io.AddMousePosEvent((float)x, (float)y);
}
