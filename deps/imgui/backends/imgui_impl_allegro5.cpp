...
// this backend.
static ImGui_ImplAllegro5_Data*
...
static void
ImGui_ImplAllegro5_SetupRenderState(ImDrawData* draw_data)
{
    // Setup blending
    al_set_separate_blender(ALLEGRO_ADD,
                            ALLEGRO_ALPHA,
...
// Render function.
void
ImGui_ImplAllegro5_RenderDrawData(ImDrawData* draw_data)
{
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++) {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
            ...
            if (pcmd->UserCallback) {
                ...
                al_set_clipping_rectangle(clip_min.x,
                                          clip_min.y,
                                          clip_max.x - clip_min.x,
                                          clip_max.y - clip_min.y);
#if ALLEGRO_HAS_DRAW_INDEXED_PRIM
                al_draw_indexed_prim(&vertices[0],
                                     bd->VertexDecl,
                                     texture,
                                     &indices[pcmd->IdxOffset],
                                     pcmd->ElemCount,
                                     ALLEGRO_PRIM_TRIANGLE_LIST);
#else
                al_draw_prim(&vertices[0],
                             bd->VertexDecl,
                             texture,
                             pcmd->IdxOffset,
...
static const char*
ImGui_ImplAllegro5_GetClipboardText(void*)
{
    ImGui_ImplAllegro5_Data* bd = ImGui_ImplAllegro5_GetBackendData();
...
static void
ImGui_ImplAllegro5_SetClipboardText(void*, const char* text)
{
    ImGui_ImplAllegro5_Data* bd = ImGui_ImplAllegro5_GetBackendData();
...
static ImGuiKey
ImGui_ImplAllegro5_KeyCodeToImGuiKey(int key_code)
{
    switch (key_code) {
...
bool
ImGui_ImplAllegro5_Init(ALLEGRO_DISPLAY* display)
{
    ImGuiIO& io = ImGui::GetIO();
...
ImGui_ImplAllegro5_UpdateKeyModifiers()
{
    ...
    io.AddKeyEvent(ImGuiMod_Alt,
                   al_key_down(&keys, ALLEGRO_KEY_ALT) ||
                     al_key_down(&keys, ALLEGRO_KEY_ALTGR));
    io.AddKeyEvent(ImGuiMod_Super,
                   al_key_down(&keys, ALLEGRO_KEY_LWIN) ||
