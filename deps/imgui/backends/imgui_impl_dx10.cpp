...
// Functions
static void
ImGui_ImplDX10_SetupRenderState(ImDrawData* draw_data, ID3D10Device* ctx)
{
...
ImGui_ImplDX10_RenderDrawData(ImDrawData* draw_data)
{
    ...
    // is (0,0) for single viewport apps.
    {
        ...
        if (bd->pVertexConstantBuffer->Map(
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++) {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
            ...
            if (pcmd->UserCallback) {
                ...
                const D3D10_RECT r = { (LONG)clip_min.x,
                                       (LONG)clip_min.y,
                                       (LONG)clip_max.x,
                                       (LONG)clip_max.y };
...
ImGui_ImplDX10_CreateFontsTexture()
{
    ...
    // Upload texture to graphics system
    {
        ...
        bd->pd3dDevice->CreateShaderResourceView(
          pTexture, &srv_desc, &bd->pFontTextureView);
        pTexture->Release();
    }
