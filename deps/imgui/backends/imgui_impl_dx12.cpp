    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++) {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
            ...
            if (pcmd->UserCallback != nullptr) {
                ...
                const D3D12_RECT r = { (LONG)clip_min.x,
                                       (LONG)clip_min.y,
                                       (LONG)clip_max.x,
                                       (LONG)clip_max.y };
...
ImGui_ImplDX12_CreateFontsTexture()
{
    ...
    // Upload texture to graphics system
    {
        ...
        bd->pd3dDevice->CreateCommittedResource(&props,
                                                D3D12_HEAP_FLAG_NONE,
                                                &desc,
        ...
        hr = bd->pd3dDevice->CreateCommandList(0,
        ...
        bd->pd3dDevice->CreateShaderResourceView(
          pTexture, &srvDesc, bd->hFontSrvCpuDescHandle);
...
ImGui_ImplDX12_CreateDeviceObjects()
{
    ...
    // Create the root signature
    {
        ...
        if (d3d12_dll == nullptr) {
            ...
            }; // A. current directory, B. used by some games, C. used in
        ...
        if (D3D12SerializeRootSignatureFn(
              &desc, D3D_ROOT_SIGNATURE_VERSION_1, &blob, nullptr) != S_OK)
            return false;

        bd->pd3dDevice->CreateRootSignature(0,
