...
//  glfwGetTime() not returning a monotonically increasing value. This seems to
//  happens on some Windows setup when peripherals disconnect, and is likely to
//  also happen on browser + Emscripten. (#6491) 2023-04-04: Inputs: Added
...
static bool
ImGui_ImplGlfw_ShouldChainCallback(GLFWwindow* window)
{
...
static bool
ImGui_ImplGlfw_Init(GLFWwindow* window,
                    ...
                    GlfwClientApi client_api)
{
    ...
    if (install_callbacks)
        ...
        // Register Emscripten Wheel callback to workaround issue in Emscripten
        // GLFW Emulation (#6096) We intentionally do not check 'if
        // (install_callbacks)' here, as some users may set it to false and call
        // GLFW callback themselves.
        // FIXME: May break chaining in case user registered their own
        // Emscripten callback?
#ifdef __EMSCRIPTEN__
    emscripten_set_wheel_callback(EMSCRIPTEN_EVENT_TARGET_DOCUMENT,
                                  nullptr,
                                  false,
                                  ImGui_ImplEmscripten_WheelCallback);
#endif
    ...
    ::SetWindowLongPtr((HWND)main_viewport->PlatformHandleRaw,
                       GWLP_WNDPROC,
    ...
    // 'docking' branch)
    {
        ...
        if (is_window_focused) {
            ...
            if (io.WantSetMousePos)
                glfwSetCursorPos(
                  window, (double)io.MousePos.x, (double)io.MousePos.y);
...
#define MAP_ANALOG(KEY_NO, _UNUSED, AXIS_NO, V0, V1)                           \
    ...
    MAP_BUTTON(
      ImGuiKey_GamepadFaceLeft, GLFW_GAMEPAD_BUTTON_X, 2); // Xbox X, PS Square
    MAP_BUTTON(
      ImGuiKey_GamepadFaceRight, GLFW_GAMEPAD_BUTTON_B, 1); // Xbox B, PS Circle
    MAP_BUTTON(
      ImGuiKey_GamepadFaceUp, GLFW_GAMEPAD_BUTTON_Y, 3); // Xbox Y, PS Triangle
    MAP_BUTTON(
      ImGuiKey_GamepadFaceDown, GLFW_GAMEPAD_BUTTON_A, 0); // Xbox A, PS Cross
    ...
    MAP_ANALOG(
      ImGuiKey_GamepadL2, GLFW_GAMEPAD_AXIS_LEFT_TRIGGER, 4, -0.75f, +1.0f);
    MAP_ANALOG(
      ImGuiKey_GamepadR2, GLFW_GAMEPAD_AXIS_RIGHT_TRIGGER, 5, -0.75f, +1.0f);
    MAP_BUTTON(ImGuiKey_GamepadL3, GLFW_GAMEPAD_BUTTON_LEFT_THUMB, 8);
    MAP_BUTTON(ImGuiKey_GamepadR3, GLFW_GAMEPAD_BUTTON_RIGHT_THUMB, 9);
    MAP_ANALOG(
      ImGuiKey_GamepadLStickLeft, GLFW_GAMEPAD_AXIS_LEFT_X, 0, -0.25f, -1.0f);
    MAP_ANALOG(
      ImGuiKey_GamepadLStickRight, GLFW_GAMEPAD_AXIS_LEFT_X, 0, +0.25f, +1.0f);
    MAP_ANALOG(
      ImGuiKey_GamepadLStickUp, GLFW_GAMEPAD_AXIS_LEFT_Y, 1, -0.25f, -1.0f);
    MAP_ANALOG(
      ImGuiKey_GamepadLStickDown, GLFW_GAMEPAD_AXIS_LEFT_Y, 1, +0.25f, +1.0f);
    MAP_ANALOG(
      ImGuiKey_GamepadRStickLeft, GLFW_GAMEPAD_AXIS_RIGHT_X, 2, -0.25f, -1.0f);
    MAP_ANALOG(
      ImGuiKey_GamepadRStickRight, GLFW_GAMEPAD_AXIS_RIGHT_X, 2, +0.25f, +1.0f);
    MAP_ANALOG(
      ImGuiKey_GamepadRStickUp, GLFW_GAMEPAD_AXIS_RIGHT_Y, 3, -0.25f, -1.0f);
    MAP_ANALOG(
      ImGuiKey_GamepadRStickDown, GLFW_GAMEPAD_AXIS_RIGHT_Y, 3, +0.25f, +1.0f);
#undef MAP_BUTTON
#undef MAP_ANALOG
}
