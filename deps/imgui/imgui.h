...
typedef int
  ImGuiPopupFlags; // -> enum ImGuiPopupFlags_      // Flags: for OpenPopup*(),
                   // BeginPopupContext*(), IsPopupOpen()
typedef int ImGuiSelectableFlags; // -> enum ImGuiSelectableFlags_ // Flags: for
                                  // Selectable()
typedef int
  ImGuiSliderFlags; // -> enum ImGuiSliderFlags_     // Flags: for DragFloat(),
...
IMGUI_API void
Render(); // ends the Dear ImGui frame, finalize the draw data. You can then get
          // call GetDrawData().
...
IMGUI_API void
ShowDemoWindow(
  bool* p_open = NULL); // create Demo window. demonstrate most ImGui features.
...
IMGUI_API void
ShowDebugLogWindow(
  bool* p_open = NULL); // create Debug Log window. display a simplified log of
...
ShowStyleEditor(
  ImGuiStyle* ref =
    NULL); // add style editor block (not a window). you can pass in a reference
           // ImGuiStyle structure to compare to, revert to and save to (else it
           // uses the default style)
IMGUI_API bool
ShowStyleSelector(
  const char* label); // add style selector block (not a window), essentially a
...
//   ImGuiChildFlags_Border == true. Consider updating your old call sites:
...
//    future update.]
IMGUI_API bool
BeginChild(const char* str_id,
           const ImVec2& size = ImVec2(0, 0),
           ImGuiChildFlags child_flags = 0,
           ImGuiWindowFlags window_flags = 0);
IMGUI_API bool
BeginChild(ImGuiID id,
           const ImVec2& size = ImVec2(0, 0),
           ImGuiChildFlags child_flags = 0,
           ImGuiWindowFlags window_flags = 0);
IMGUI_API void
...
IMGUI_API bool
IsWindowFocused(
  ImGuiFocusedFlags flags = 0); // is current window focused? or its root/child,
...
IsWindowHovered(
  ImGuiHoveredFlags flags =
    0); // is current window hovered and hoverable (e.g. not blocked by a
        // popup/modal)? See ImGuiHoveredFlags_ for options. IMPORTANT: If you
        // are trying to check whether your mouse should be dispatched to Dear
        // ImGui or to your underlying app, you should not use this function!
        // Use the 'io.WantCaptureMouse' boolean for that! Refer to FAQ entry
        // "How can I tell whether to dispatch mouse/keyboard to Dear ImGui or
        // my application?" for details.
...
IMGUI_API void
SetNextWindowPos(const ImVec2& pos,
                 ImGuiCond cond = 0,
...
SetNextWindowSizeConstraints(
  ...
  void* custom_callback_data =
    NULL); // set next window size limits. use 0.0f or FLT_MAX if you don't want
           // limits. Use -1 for both min and max of same axis to preserve
           // current size (which itself is a constraint). Use callback to apply
...
IMGUI_API void
SetNextWindowFocus(); // set next window to be focused / top-most. call before
                      // Begin()
IMGUI_API void
SetNextWindowScroll(
...
SetNextWindowBgAlpha(
  float
    alpha); // set next window background color alpha. helper to easily override
            // the Alpha component of ImGuiCol_WindowBg/ChildBg/PopupBg. you may
            // also use ImGuiWindowFlags_NoBackground.
IMGUI_API void
SetWindowPos(
  const ImVec2& pos,
...
IMGUI_API void
PushButtonRepeat(
  bool repeat); // in 'repeat' mode, Button*() functions return repeated true in
...
SetNextItemWidth(
  float
    item_width); // set width of the _next_ common large "item+label" widget.
                 // >0.0f: width in pixels, <0.0f align xx pixels to the right
                 // of window (so -FLT_MIN always align width to the right side)
IMGUI_API float
...
PushTextWrapPos(
  float wrap_local_pos_x =
    0.0f); // push word-wrapping position for Text*() commands. < 0.0f: no
           // wrapping; 0.0f: wrap to end of window (or column); > 0.0f: wrap at
...
IMGUI_API float
GetFontSize(); // get current font size (= height in pixels) of current font
...
GetStyleColorVec4(
  ImGuiCol idx); // retrieve style color as stored in ImGuiStyle structure. use
...
IMGUI_API ImVec2
GetCursorPos(); // [window-local] cursor position in window coordinates
...
IMGUI_API void
SetCursorPos(const ImVec2& local_pos); // [window-local] "
IMGUI_API void
SetCursorPosX(float local_x); // [window-local] "
IMGUI_API void
SetCursorPosY(float local_y); // [window-local] "
IMGUI_API ImVec2
GetCursorStartPos(); // [window-local] initial cursor position, in window
                     // coordinates
...
IMGUI_API void
NewLine(); // undo a SameLine() or force a new line when in a horizontal-layout
           // context.
IMGUI_API void
...
IMGUI_API void
TextV(const char* fmt, va_list args) IM_FMTLIST(1);
IMGUI_API void
TextColored(const ImVec4& col, const char* fmt, ...)
  IM_FMTARGS(2); // shortcut for PushStyleColor(ImGuiCol_Text, col); Text(fmt,
                 // ...); PopStyleColor();
IMGUI_API void
TextColoredV(const ImVec4& col, const char* fmt, va_list args) IM_FMTLIST(2);
IMGUI_API void
TextDisabled(const char* fmt, ...) IM_FMTARGS(
  1); // shortcut for PushStyleColor(ImGuiCol_Text,
      // style.Colors[ImGuiCol_TextDisabled]); Text(fmt, ...); PopStyleColor();
IMGUI_API void
TextDisabledV(const char* fmt, va_list args) IM_FMTLIST(1);
IMGUI_API void
TextWrapped(const char* fmt, ...) IM_FMTARGS(
  1); // shortcut for PushTextWrapPos(0.0f); Text(fmt, ...); PopTextWrapPos();.
      // Note that this won't work on an auto-resizing window if there's no
      // other widgets to extend the window width, yoy may need to set a size
      // using SetNextWindowSize().
IMGUI_API void
TextWrappedV(const char* fmt, va_list args) IM_FMTLIST(1);
IMGUI_API void
LabelText(const char* label, const char* fmt, ...) IM_FMTARGS(
  2); // display text+label aligned the same way as value+label widgets
IMGUI_API void
LabelTextV(const char* label, const char* fmt, va_list args) IM_FMTLIST(2);
IMGUI_API void
BulletText(const char* fmt, ...) IM_FMTARGS(1); // shortcut for Bullet()+Text()
IMGUI_API void
BulletTextV(const char* fmt, va_list args) IM_FMTLIST(1);
IMGUI_API void
SeparatorText(
...
IMGUI_API bool
Button(const char* label, const ImVec2& size = ImVec2(0, 0)); // button
IMGUI_API bool
SmallButton(const char* label); // button with (FramePadding.y == 0) to easily
                                // embed within text
IMGUI_API bool
InvisibleButton(
  const char* str_id,
  ...
  ImGuiButtonFlags flags =
    0); // flexible button behavior without the visuals, frequently useful to
        // build custom behaviors using the public api (along with IsItemActive,
        // IsItemHovered, etc.)
IMGUI_API bool
...
RadioButton(const char* label,
            int* v,
            int v_button); // shortcut to handle the above pattern when value is
                           // an integer
IMGUI_API void
...
IMGUI_API void
Image(ImTextureID user_texture_id,
      const ImVec2& size,
...
DragFloat4(const char* label,
           ...
           ImGuiSliderFlags flags = 0);
IMGUI_API bool
DragFloatRange2(const char* label,
                float* v_current_min,
                float* v_current_max,
                float v_speed = 1.0f,
                float v_min = 0.0f,
...
DragInt4(const char* label,
         ...
         ImGuiSliderFlags flags = 0);
IMGUI_API bool
DragIntRange2(const char* label,
              int* v_current_min,
              int* v_current_max,
              float v_speed = 1.0f,
              int v_min = 0,
...
SliderScalar(const char* label,
             ...
             ImGuiSliderFlags flags = 0);
IMGUI_API bool
SliderScalarN(const char* label,
              ImGuiDataType data_type,
              void* p_data,
              int components,
              const void* p_min,
...
VSliderInt(const char* label,
           ...
           ImGuiSliderFlags flags = 0);
IMGUI_API bool
VSliderScalar(const char* label,
              const ImVec2& size,
              ImGuiDataType data_type,
              void* p_data,
              const void* p_min,
...
InputFloat(const char* label,
           ...
           ImGuiInputTextFlags flags = 0);
IMGUI_API bool
InputFloat2(const char* label,
            float v[2],
            const char* format = "%.3f",
            ImGuiInputTextFlags flags = 0);
IMGUI_API bool
InputFloat3(const char* label,
            float v[3],
            const char* format = "%.3f",
            ImGuiInputTextFlags flags = 0);
IMGUI_API bool
InputFloat4(const char* label,
            float v[4],
            const char* format = "%.3f",
            ImGuiInputTextFlags flags = 0);
IMGUI_API bool
...
SetColorEditOptions(
  ImGuiColorEditFlags
    flags); // initialize current options (generally on application startup) if
            // you want to select a default format, picker type, etc. User will
            // be able to change many settings, unless you pass the _NoOptions
            // flag to your calls.
...
IMGUI_API bool
TreeNode(const char* str_id, const char* fmt, ...) IM_FMTARGS(
  2); // helper variation to easily decorelate the id from the displayed string.
      // Read the FAQ about why and how to use ID. to align arbitrary text at
      // the same level as a TreeNode() you can use Bullet().
IMGUI_API bool
TreeNode(const void* ptr_id, const char* fmt, ...) IM_FMTARGS(2); // "
IMGUI_API bool
TreeNodeV(const char* str_id, const char* fmt, va_list args) IM_FMTLIST(2);
IMGUI_API bool
TreeNodeV(const void* ptr_id, const char* fmt, va_list args) IM_FMTLIST(2);
IMGUI_API bool
...
TreeNodeEx(const void* ptr_id, ImGuiTreeNodeFlags flags, const char* fmt, ...)
  IM_FMTARGS(3);
IMGUI_API bool
TreeNodeExV(const char* str_id,
            ImGuiTreeNodeFlags flags,
            const char* fmt,
            va_list args) IM_FMTLIST(3);
IMGUI_API bool
TreeNodeExV(const void* ptr_id,
            ImGuiTreeNodeFlags flags,
            const char* fmt,
            va_list args) IM_FMTLIST(3);
IMGUI_API void
...
CollapsingHeader(
  const char* label,
  ImGuiTreeNodeFlags flags =
    0); // if returning 'true' the header is open. doesn't indent nor push on ID
...
CollapsingHeader(
  ...
  ImGuiTreeNodeFlags flags =
    0); // when 'p_visible != NULL': if '*p_visible==true' display an additional
        // small close button on upper right of the header which will set the
        // bool to false when clicked, if '*p_visible==false' don't display the
        // header.
IMGUI_API void
SetNextItemOpen(
  bool is_open,
...
IMGUI_API void
EndMainMenuBar(); // only call EndMainMenuBar() if BeginMainMenuBar() returns
                  // true!
IMGUI_API bool
...
IMGUI_API void
EndTooltip(); // only call EndTooltip() if BeginTooltip()/BeginItemTooltip()
              // returns true!
IMGUI_API void
SetTooltip(const char* fmt, ...) IM_FMTARGS(
  1); // set a text-only tooltip. Often used after a ImGui::IsItemHovered()
      // check. Override any previous call to SetTooltip().
IMGUI_API void
SetTooltipV(const char* fmt, va_list args) IM_FMTLIST(1);
...
IMGUI_API bool
BeginItemTooltip(); // begin/append a tooltip window if preceding item was
                    // hovered.
IMGUI_API void
...
IMGUI_API void
SetItemTooltipV(const char* fmt, va_list args) IM_FMTLIST(1);
...
// scrolled.
IMGUI_API void
TableSetupColumn(const char* label,
...
IMGUI_API void
Columns(int count = 1, const char* id = NULL, bool border = true);
IMGUI_API void
NextColumn(); // next column, defaults to current row or next row if the current
              // row is finished
IMGUI_API int
GetColumnIndex(); // get current column index
IMGUI_API float
GetColumnWidth(int column_index = -1); // get column width (in pixels). pass -1
...
SetTabItemClosed(
  const char*
    tab_or_docked_window_label); // notify TabBar or Docking system of a closed
                                 // tab/window ahead (useful to reduce visual
                                 // flicker on reorderable tab bars). For
                                 // tab-bar: call after BeginTabBar() and before
                                 // Tab submissions. Otherwise call with a
                                 // window name.
...
IMGUI_API void
LogTextV(const char* fmt, va_list args) IM_FMTLIST(1);
...
IMGUI_API void
EndDragDropSource(); // only call EndDragDropSource() if BeginDragDropSource()
                     // returns true!
IMGUI_API bool
...
AcceptDragDropPayload(
  ...
  ImGuiDragDropFlags flags =
    0); // accept contents of a given type. If
        // ImGuiDragDropFlags_AcceptBeforeDelivery is set you can peek into the
        // payload before the mouse button is released.
IMGUI_API void
EndDragDropTarget(); // only call EndDragDropTarget() if BeginDragDropTarget()
                     // returns true!
IMGUI_API const ImGuiPayload*
...
IMGUI_API void
PushClipRect(const ImVec2& clip_rect_min,
...
IMGUI_API bool
IsItemVisible(); // is the last item visible? (items may be out of sight because
...
IMGUI_API bool
IsItemActivated(); // was the last item just made active (item was previously
                   // inactive).
IMGUI_API bool
...
IMGUI_API ImGuiID
GetItemID(); // get ID of last item (~~ often same ImGui::GetID(label)
             // beforehand)
IMGUI_API ImVec2
GetItemRectMin(); // get upper-left bounding rectangle of the last item (screen
                  // space)
IMGUI_API ImVec2
GetItemRectMax(); // get lower-right bounding rectangle of the last item (screen
                  // space)
IMGUI_API ImVec2
...
IMGUI_API ImGuiStorage*
GetStateStorage();

// Text Utilities
IMGUI_API ImVec2
CalcTextSize(const char* text,
...
IMGUI_API bool
IsKeyDown(ImGuiKey key); // is key being held.
IMGUI_API bool
IsKeyPressed(
  ImGuiKey key,
...
IsKeyChordPressed(
  ImGuiKeyChord
    key_chord); // was key chord (mods + key) pressed, e.g. you can pass
                // 'ImGuiMod_Ctrl | ImGuiKey_S' as a key-chord. This doesn't do
                // any routing or focus check, please consider using Shortcut()
                // function instead.
IMGUI_API int
GetKeyPressedAmount(
  ImGuiKey key,
  ...
  float rate); // uses provided repeat rate/delay. return a count, most often 0
               // or 1 but might be >1 if RepeatRate is small enough that
...
SetNextFrameWantCaptureKeyboard(
  bool
    want_capture_keyboard); // Override io.WantCaptureKeyboard flag next frame
                            // (said flag is left for your application to
                            // handle, typically when true it instructs your app
                            // to ignore inputs). e.g. force capture keyboard
                            // when your widget is being hovered. This is
                            // equivalent to setting "io.WantCaptureKeyboard =
                            // want_capture_keyboard"; after the next NewFrame()
                            // call.
...
IsMouseDoubleClicked(
  ImGuiMouseButton
    button); // did mouse button double-clicked? Same as GetMouseClickedCount()
             // == 2. (note that a double-click will also report
             // IsMouseClicked() == true)
IMGUI_API int
...
IMGUI_API ImVec2
GetMousePos(); // shortcut to ImGui::GetIO().MousePos provided by user, to be
...
GetMouseDragDelta(
  ...
  float lock_threshold =
    -1.0f); // return the delta from the initial clicking position while the
            // mouse button is pressed or was just released. This is locked and
            // return 0.0f until the mouse moves past a distance threshold at
            // least once (if lock_threshold < -1.0f, uses
            // io.MouseDraggingThreshold)
IMGUI_API void
...
SetNextFrameWantCaptureMouse(
  bool
    want_capture_mouse); // Override io.WantCaptureMouse flag next frame (said
                         // flag is left for your application to handle, typical
                         // when true it instucts your app to ignore inputs).
...
LoadIniSettingsFromDisk(
  const char* ini_filename); // call after CreateContext() and before the first
...
LoadIniSettingsFromMemory(
  const char* ini_data,
  size_t ini_size =
    0); // call after CreateContext() and before the first call to NewFrame() to
...
IMGUI_API void
SetAllocatorFunctions(ImGuiMemAllocFunc alloc_func,
                      ImGuiMemFreeFunc free_func,
                      void* user_data = NULL);
IMGUI_API void
GetAllocatorFunctions(ImGuiMemAllocFunc* p_alloc_func,
...
// (Legacy: bot 0 must always correspond to ImGuiChildFlags_Border to be
...
enum ImGuiKey : int
{
    ...
    // their own native index + the io.KeyMap[] array. We are ditching this
    // method but keeping a legacy path for user code doing e.g.
    // IsKeyPressed(MY_NATIVE_KEY_CODE) If you need to iterate all keys (for
    // e.g. an input mapper) you may use
...
{};
inline void*
operator new(size_t, ImNewWrapper, void* ptr)
{
    return ptr;
}
inline void
operator delete(void*, ImNewWrapper, void*)
{
...
struct ImGuiStyle
{
    ...
    ImVec2 WindowMinSize;   // Minimum window size. This is a global setting. If
                            ...
                            // SetNextWindowSizeConstraints().
    ImVec2
      WindowTitleAlign; // Alignment for title bar text. Defaults to (0.0f,0.5f)
                        // for left-aligned,vertically centered.
    ...
    ImVec2
      SeparatorTextAlign; // Alignment of text within the separator. Defaults to
...
struct ImGuiIO
{
    ...
    const char*
      IniFilename; // = "imgui.ini"    // Path to .ini file (important: default
                   // "imgui.ini" is relative to current working dir!). Set NULL
                   // to disable automatic .ini loading/saving or if you want to
                   // manually call LoadIniSettingsXXX() / SaveIniSettingsXXX()
                   // functions.
    const char*
    ...
    // users. We expect to update the API eventually. In the meanwhile we
    // provide tools to facilitate checking user-code behavior.
    ...
    // interactions with a debugger when focus loss leads to clearing inputs
    // data. Backends may have other side-effects on focus loss, so this will
    // reduce side-effects but not necessary remove all of them. Consider using
    // e.g. Win32's IsDebuggerPresent() as an additional filter (or see
    // ImOsIsDebuggerPresent() in imgui_test_engine/imgui_te_utils.cpp for a
    // Unix compatible version).
    bool ConfigDebugIgnoreFocusLoss; // = false          // Ignore
                                     // io.AddFocusEvent(false), consequently
                                     // not calling io.ClearInputKeys() in input
                                     // processing.
    ...
    IMGUI_API void SetAppAcceptingEvents(
      bool
        accepting_events); // Set master flag for accepting key/mouse/text
    ...
    IMGUI_API void
    ClearInputKeys(); // Clear current keyboard/mouse/gamepad state + current
                      // frame text input buffer. Equivalent to releasing all
                      // keys/buttons.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    IMGUI_API void
    ...
    float NavInputs[ImGuiNavInput_COUNT]; // [LEGACY] Since 1.88, NavInputs[]
                                          // was removed. Backends from 1.60
                                          // to 1.86 won't build. Feed gamepad
                                          // inputs via io.AddKeyEvent() and
...
struct ImGuiListClipper
{
    ...
    IMGUI_API void IncludeItemsByIndex(
      int item_begin,
      int item_end); // item_end is exclusive e.g. use (42, 42+1) to make item
                     // 42 never clipped.
...
operator*(const ImVec2& lhs, const float rhs)
{
...
static inline ImVec2&
operator+=(ImVec2& lhs, const ImVec2& rhs)
{
    lhs.x += rhs.x;
    lhs.y += rhs.y;
    return lhs;
...
operator+(const ImVec4& lhs, const ImVec4& rhs)
{
...
struct ImDrawList
{
    ...
    ImDrawCmdHeader
      _CmdHeader; // [Internal] template of active commands. Fields should match
    ...
    // ImVec2& p3, const ImVec2& p4, ImU32 col, float thickness, int
    // num_segments = 0) { AddBezierCubic(p1, p2, p3, p4, col, thickness,
    // num_segments); } // OBSOLETED in 1.80 (Jan 2021) inline  void
    // PathBezierCurveTo(const ImVec2& p2, const ImVec2& p3, const ImVec2& p4,
...
struct ImDrawData
{
    ...
    int CmdListsCount; // Number of ImDrawList* to render (should always be ==
                       // CmdLists.size)
    int
      TotalIdxCount; // For convenience, sum of all ImDrawList's IdxBuffer.Size
    int
      TotalVtxCount; // For convenience, sum of all ImDrawList's VtxBuffer.Size
    ImVector<ImDrawList*>
...
struct ImFontConfig
{
    ...
    const ImWchar*
      GlyphRanges; // NULL     // THE ARRAY DATA NEEDS TO PERSIST AS LONG AS THE
                   // FONT IS ALIVE. Pointer to a user-provided list of Unicode
...
struct ImFontAtlas
{
    ...
    IMGUI_API ImFont* AddFontFromMemoryTTF(
      ...
      const ImWchar* glyph_ranges =
        NULL); // Note: Transfer ownership of 'ttf_data' to ImFontAtlas! Will be
               ...
               // font_cfg->FontDataOwnedByAtlas=false to keep ownership of your
               // data and it won't be freed.
    ...
    IMGUI_API ImFont* AddFontFromMemoryCompressedBase85TTF(
      ...
      const ImFontConfig* font_cfg = NULL,
      const ImWchar* glyph_ranges =
        NULL); // 'compressed_font_data_base85' still owned by caller. Compress
    ...
    const ImFontBuilderIO*
      FontBuilderIO; // Opaque interface to a font builder (default to
                     // stb_truetype, can be changed to use FreeType by defining
...
static inline void
ShowStackToolWindow(bool* p_open = NULL)
{
...
static inline void
PushAllowKeyboardFocus(bool tab_stop)
{
...
IMGUI_API bool
ImageButton(ImTextureID user_texture_id,
            const ImVec2& size,
            const ImVec2& uv0 = ImVec2(0, 0),
            const ImVec2& uv1 = ImVec2(1, 1),
            int frame_padding = -1,
            ...
            const ImVec4& tint_col =
              ImVec4(1, 1, 1, 1)); // Use new ImageButton() signature (explicit
                                   // item id, regular FramePadding)
// OBSOLETED in 1.88 (from May 2022)
static inline void
CaptureKeyboardFromApp(bool want_capture_keyboard = true)
{
