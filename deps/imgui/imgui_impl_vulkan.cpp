...
void
ImGui_ImplVulkanH_DestroyFrame(VkDevice device,
...
ImGui_ImplVulkanH_DestroyWindowRenderBuffers(
  ...
  const VkAllocationCallbacks* allocator);
void
ImGui_ImplVulkanH_CreateWindowSwapChain(VkPhysicalDevice physical_device,
                                        VkDevice device,
...
static uint32_t
ImGui_ImplVulkan_MemoryType(VkMemoryPropertyFlags properties,
                            uint32_t type_bits)
{
...
static void
CreateOrResizeBuffer(VkBuffer& buffer,
...
static void
ImGui_ImplVulkan_SetupRenderState(ImDrawData* draw_data,
                                  VkPipeline pipeline,
...
ImGui_ImplVulkan_CreateFontsTexture()
{
    ...
    if (bd->FontCommandPool == VK_NULL_HANDLE) {
        ...
        vkCreateCommandPool(
          v->Device, &info, v->Allocator, &bd->FontCommandPool);
    }
    if (bd->FontCommandBuffer == VK_NULL_HANDLE) {
        ...
        err =
          vkAllocateCommandBuffers(v->Device, &info, &bd->FontCommandBuffer);
        check_vk_result(err);
    }
    ...
    // Create the Image:
    {
        ...
        err = vkAllocateMemory(
          v->Device, &alloc_info, v->Allocator, &bd->FontMemory);
        check_vk_result(err);
    ...
    VkBuffer upload_buffer;
    {
        ...
        alloc_info.memoryTypeIndex = ImGui_ImplVulkan_MemoryType(
          VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, req.memoryTypeBits);
        err = vkAllocateMemory(
          v->Device, &alloc_info, v->Allocator, &upload_buffer_memory);
        check_vk_result(err);
        err =
    ...
    // Upload to Buffer:
    {
        char* map = nullptr;
        err = vkMapMemory(
          v->Device, upload_buffer_memory, 0, upload_size, 0, (void**)(&map));
        check_vk_result(err);
                                     ...
                                     const VkAllocationCallbacks* allocator)
{
    ...
    if (bd->ShaderModuleVert == VK_NULL_HANDLE) {
        ...
        VkResult err = vkCreateShaderModule(
          device, &vert_info, allocator, &bd->ShaderModuleVert);
        check_vk_result(err);
    }
    if (bd->ShaderModuleFrag == VK_NULL_HANDLE) {
        ...
        VkResult err = vkCreateShaderModule(
          device, &frag_info, allocator, &bd->ShaderModuleFrag);
        check_vk_result(err);
    }
}

static void
ImGui_ImplVulkan_CreatePipeline(VkDevice device,
                                const VkAllocationCallbacks* allocator,
                                ...
                                uint32_t subpass)
{
    ...
    VkResult err = vkCreateGraphicsPipelines(
      device, pipelineCache, 1, &info, allocator, pipeline);
    check_vk_result(err);
}
...
ImGui_ImplVulkan_CreateDeviceObjects()
{
    ...
    if (!bd->DescriptorSetLayout) {
        ...
        err = vkCreateDescriptorSetLayout(
          v->Device, &info, v->Allocator, &bd->DescriptorSetLayout);
        check_vk_result(err);
    }
    ...
    if (!bd->PipelineLayout) {
        ...
        err = vkCreatePipelineLayout(
          v->Device, &layout_info, v->Allocator, &bd->PipelineLayout);
        check_vk_result(err);
    }
...
ImGui_ImplVulkan_DestroyDeviceObjects()
{
    ...
    if (bd->DescriptorSetLayout) {
        vkDestroyDescriptorSetLayout(
          v->Device, bd->DescriptorSetLayout, v->Allocator);
                                      ...
                                      VkColorSpaceKHR request_color_space)
{
    ...
    vkGetPhysicalDeviceSurfaceFormatsKHR(
      physical_device, surface, &avail_count, nullptr);
                                        ...
                                        uint32_t min_image_count)
{
    ...
    // Create Swapchain
    {
        ...
        err = vkGetSwapchainImagesKHR(
          device, wd->Swapchain, &wd->ImageCount, nullptr);
        check_vk_result(err);
    ...
    // Create The Image Views
    {
        ...
        for (uint32_t i = 0; i < wd->ImageCount; i++) {
            ...
            err =
              vkCreateImageView(device, &info, allocator, &fd->BackbufferView);
            check_vk_result(err);
        }
    }
...
void
ImGui_ImplVulkanH_DestroyFrameSemaphores(VkDevice device,
                                         ImGui_ImplVulkanH_FrameSemaphores* fsd,
                                         const VkAllocationCallbacks* allocator)
{
