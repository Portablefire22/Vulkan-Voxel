...
VulkanEngine::init()
{
    ...
    // if (enableValidationLayers) { std::cout << "Validation Layers
    // Enabled!\n";
    // }
    ...
    _window = SDL_CreateWindow("Vulkan Voxel",
...
Material*
VulkanEngine::createMaterial(VkPipeline pipeline,
...
Material*
VulkanEngine::getMaterial(const std::string& name)
{
...
Mesh*
VulkanEngine::getMesh(const std::string& name)
{
...
void
VulkanEngine::drawObjects(VkCommandBuffer cmd, RenderObject* first, int count)
{
    // Get a model view matrix
    // These values aren't used directly but the functions refresh the values
    // for use. Without these, it just becomes 2D
    PlayerEntity.camera.GetViewMatrix();
    PlayerEntity.camera.getProjectionMatrix(_windowExtent.width,
                                            _windowExtent.height);
    ...
    void* objectData;
    vmaMapMemory(
      _allocator, getCurrentFrame().objectBuffer._allocation, &objectData);
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (object.material != lastMaterial) {
            ...
            vkCmdBindDescriptorSets(cmd,
                                    ...
                                    1,
                                    1,
                                    &getCurrentFrame().objectDescriptor,
                                    0,
                                    nullptr);
...
void
VulkanEngine::uploadMesh(Mesh& mesh)
{
    ...
    immediateSubmit(([=, this](VkCommandBuffer cmd) {
        ...
        vkCmdCopyBuffer(
          cmd, stagingBuffer._buffer, mesh._vertexBuffer._buffer, 1, &copy);
    }));
    ...
    vmaDestroyBuffer(
      _allocator, stagingBuffer._buffer, stagingBuffer._allocation);
}

void
VulkanEngine::loadImages(std::string Path, std::string Name)
{
    Texture temp;
...
VulkanEngine::run()
{
    ...
    while (!bQuit) {
        ...
        deltaTime = (double)(nowFrameTime - lastFrameTime) * 1000 /
        ...
        while (SDL_PollEvent(&e) != 0) {
            ...
            switch (e.type) {
                ...
                case SDL_WINDOWEVENT:
                    if (e.window.event ==
                        SDL_WINDOWEVENT_SIZE_CHANGED) { // TODO HANDLE RESIZING
...
VulkanEngine::initVulkan()
{
    ...
    vkb::Device vkbDevice =
      deviceBuilder.add_pNext(&shaderDrawParametersFeatures).build().value();
    _device = vkbDevice;
    ...
    std::cout << "Minimum Buffer Alignment : "
...
size_t
VulkanEngine::padUniformBufferSize(size_t originalSize)
{
...
VulkanEngine::initSwapchain()
{
    ...
    VkImageCreateInfo depthImageInfo =
      vkInit::imageCreateInfo(_depthFormat,
...
VulkanEngine::initCommands()
{
    ...
    VkCommandPoolCreateInfo uploadCommandPoolInfo =
      vkInit::commandPoolCreateInfo(_graphicsQueueFamily);
    VK_CHECK(vkCreateCommandPool(
    ...
    VkCommandBufferAllocateInfo cmdAllocInfo =
      vkInit::commandBufferAllocateInfo(_uploadContext._commandPool, 1);
    VkCommandBuffer cmd;
    VK_CHECK(vkAllocateCommandBuffers(
      _device, &cmdAllocInfo, &_uploadContext._commandBuffer));
}

// Immediate execution of command to GPU
void
VulkanEngine::immediateSubmit(
...
void
VulkanEngine::initDefaultRenderpass()
{
    ...
    VkAttachmentDescription attachments[2] = { colourAttachment,
                                               depthAttachment };
    ...
    if (vkCreateRenderPass(_device, &renderPassInfo, nullptr, &_renderPass) !=
        ...
        throw std::runtime_error(
          "initDefaultRenderPass(): Failed to create render pass!");
    }
...
VulkanEngine::initFramebuffers()
{
    ...
    std::cout << frameBufferInfo.width << " | " << frameBufferInfo.height
              << std::endl;
    ...
    for (int i = 0; i < swapChainImageCount; i++) {
        ...
        if (vkCreateFramebuffer(
            ...
            throw std::runtime_error(
              "initFramebuffers(): Failed to create framebuffer!");
        }
        _mainDeletionQueue.push_function([=, this]() {
...
void
VulkanEngine::initSyncStructures()
{
    ...
    VK_CHECK(vkCreateFence(
      _device, &uploadFenceCreateInfo, nullptr, &_uploadContext._uploadFence));
    _mainDeletionQueue.push_function([=, this]() {
...
VulkanEngine::draw()
{
    ...
    vkWaitForFences(_device,
                    ...
                    true,
                    1000000000); // 1 second timeout, nanoseconds
    ...
    vkAcquireNextImageKHR(_device,
                          _swapchain,
                          1000000000,
                          getCurrentFrame()._presentSemaphore,
                          nullptr,
                          &swapchainImageIndex);
...
AllocatedBuffer
VulkanEngine::createBuffer(size_t allocSize,
...
VulkanEngine::initDescriptors()
{
    ...
    VkDescriptorSetLayoutBinding bindings[] = { cameraBufferBinding,
                                                sceneBind };
    ...
    vkCreateDescriptorSetLayout(
      _device, &set3info, nullptr, &_singleTextureSetLayout);

    for (int i = 0; i < FRAME_OVERLAP; i++) {
        _frames[i].cameraBuffer =
          createBuffer(sizeof(GPUCameraData),
                       VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                       VMA_MEMORY_USAGE_CPU_TO_GPU);
        const int MAX_OBJECTS = 100000;
        _frames[i].objectBuffer =
          createBuffer(sizeof(GPUObjectData) * MAX_OBJECTS,
        ...
        vkAllocateDescriptorSets(
          _device, &allocInfo, &_frames[i].globalDescriptor);
        ...
        vkAllocateDescriptorSets(
          _device, &objectSetAlloc, &_frames[i].objectDescriptor);
        ...
        VkWriteDescriptorSet setWrites[] = { cameraWrite,
                                             sceneWrite,
                                             objectWrite };
...
bool
VulkanEngine::loadShaderModule(const char* filePath,
...
VulkanEngine::initPipelines()
{
    ...
    if (!loadShaderModule("../shaders/frag.spv", &texturedMeshShader)) {
        ...
        std::cout << "Textured mesh fragment shader succesfully loaded"
                  << std::endl;
    }
    ...
    pipelineBuilder._inputAssembly =
      vkInit::inputAssemblyStateCreateInfo(VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST);
    ...
    pipelineBuilder._colourBlendAttachment =
      vkInit::colourBlendAttachmentState();
    ...
    vkCreatePipelineLayout(
      _device, &meshPipelineLayoutInfo, nullptr, &_meshPipelineLayout);
...
void
VulkanEngine::proccess_mem_usage(double& vm_usage, double& resident_set)
{
    using std::ifstream;
    using std::ios_base;
    using std::string;
    ...
    long page_size_kb = sysconf(
      _SC_PAGE_SIZE / 1024); // in case x86_64 is configured to use 2MB pages
...
VkPipeline
PipelineBuilder::buildPipeline(VkDevice device, VkRenderPass pass)
{
