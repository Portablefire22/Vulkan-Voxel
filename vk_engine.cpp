...
void VulkanEngine::init() {
  ...
  _window = SDL_CreateWindow("Vulkan Voxel", SDL_WINDOWPOS_UNDEFINED,
...
void VulkanEngine::drawObjects(VkCommandBuffer cmd, RenderObject *first,
  ...
  // These values aren't used directly but the functions refresh the values for
  // use. Without these, it just becomes 2D
  ...
  void *objectData;
  vmaMapMemory(_allocator, getCurrentFrame().objectBuffer._allocation,
               &objectData);
...
void VulkanEngine::initScene() {
  ...
  chunksToRender =
      currentWorld.GetChunksAroundPlayer(*this, PlayerEntity, 2, 2);
...
void VulkanEngine::initSwapchain() {
  ...
  VkImageCreateInfo depthImageInfo = vkInit::imageCreateInfo(
  ...
  vmaCreateImage(_allocator, &depthImageInfo, &depthImageAllocInfo,
...
void VulkanEngine::draw() {
  ...
  vkWaitForFences(_device, 1, &getCurrentFrame()._renderFence, true,
  ...
  vkAcquireNextImageKHR(_device, _swapchain, 1000000000,
                        getCurrentFrame()._presentSemaphore, nullptr,
                        &swapchainImageIndex);
...
void VulkanEngine::initDescriptors() {
  ...
  VkDescriptorSetLayoutBinding textureBind = vkInit::descriptorSetLayoutBinding(
  ...
  for (int i = 0; i < FRAME_OVERLAP; i++) {
    ...
    _frames[i].objectBuffer = createBuffer(sizeof(GPUObjectData) * MAX_OBJECTS,
...
void VulkanEngine::initPipelines() {
  ...
  pipelineBuilder._shaderStages.push_back(vkInit::pipelineShaderStageCreateInfo(
  ...
  vkCreatePipelineLayout(_device, &meshPipelineLayoutInfo, nullptr,
                         &_meshPipelineLayout);
...
void VulkanEngine::proccess_mem_usage(double &vm_usage, double &resident_set) {
  using std::ifstream;
  using std::ios_base;
  using std::string;
  ...
  long page_size_kb = sysconf(_SC_PAGE_SIZE /
                      1024); // in case x86_64 is configured to use 2MB pages
